...
@RestController
@RequestMapping("/data")
public class Question33Controller extends ApiController {
    @Resource
    private New1qualityProblemDataService new1qualityProblemDataService;
    @Resource
    private NewTableFlagService newTableFlagService;
    @RequestMapping("/question33")
    public Map<String, Object> question33() {
//        if(rate<=0) rate=1.5;
        //故障最早发生日期
        ...
        for (New1qualityProblemData new1qualityProblemData : dataList) {
            Date time = new1qualityProblemData.getFaultpartsDeliverytime();
...
//            System.out.println(time);
        ...
        //遍历故障类型次
        List<Question33ChartData> series = new ArrayList<>();
        for (int i = 0; i < titles.size(); i++) {
            ...
            series.add(new Question33ChartData("bar", strs,titleName));
        ...
        for (int i = 0; i < titles.size(); i++) {
            int count = new1qualityProblemDataService.count(new QueryWrapper<New1qualityProblemData>()
                    .eq("faultyparts_title", titles.get(i).getFaultypartsTitle()));
...
//
//                    Question33ChartData question33ChartData = series.get(i);
//                    int[] data = question33ChartData.getData();
//                    data[j]=0;
//                    question33ChartData.setData(data);
//                    series.set(i, question33ChartData);
//                    signal[j]++;
//                }
//            }
//        }
...
//        }
    ...
    @RequestMapping("/question33/year")
    public Map<String, Object> question33Year() {
//        if(rate<=0) rate=1.5;
        //故障最早发生日期
        Date earlyTime = new Date();
        //故障最晚发生日期
...
//        }
    ...
    @RequestMapping("/question33/table")
...
//                ints = new int[1];
//            }
            Question33ChartData data = series.get(i);
            data.setData(ints);
            series.set(i, data);
...
//        System.out.println(list);
        ...
        for (New1qualityProblemData new1qualityProblemData : dataList) {
            ...
            for (NameTimeNum nameTimeNum : list) {
                Date date = new1qualityProblemData.getFaultpartsDeliverytime();
//                System.out.println(date);
            ...
            if (sign==0&&new1qualityProblemData.getFaultpartsDeliverytime().after(new Date(49,1,1))) {
...
class Question33ChartData implements Serializable {
    public String type;
    public int[] data;
    public String name;

    public Question33ChartData(String type, int[] data, String name) {
        this.type = type;
        this.data = data;
        this.name = name;
    }

    public Question33ChartData() {
    }
