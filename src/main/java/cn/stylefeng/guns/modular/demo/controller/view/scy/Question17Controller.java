...
@RestController
@RequestMapping("/data")
public class Question17Controller extends ApiController {

    @Resource
    private New1qualityProblemDataService new1qualityProblemDataService;

    @RequestMapping("/question17")
    public Map<String, Object> question17() {
        //故障最早发生日期
        ...
        //获取故障件类型
        List<New1qualityProblemData> batch = new1qualityProblemDataService.list(new QueryWrapper<New1qualityProblemData>()
                .select("zb_batch")
                .groupBy("zb_batch")
                .orderByDesc("zb_batch")
        );
        typeList = new String[batch.size()];
        //获取所有数据项
...
//            System.out.println(time);
        ...
        //遍历故障类型次
        List<question17ChartData> series = new ArrayList<>();
        for (int i = 0; i < batch.size(); i++) {
            //创建类型数量个数组 数组长度为
            //创建区间数组
            int[] strs = new int[range];
            String typeName = batch.get(i).getZbBatch();
            //遍历全部数据记录
            for (New1qualityProblemData new1qualityProblemData : dataList) {
                ...
                if (time.after(new Date(49,1,1))) {
                    ...
                    if (new1qualityProblemData.getZbBatch().equals(typeName)) {
                        strs[year * 12 + month]++;
                    }
                }
            }
            series.add(new question17ChartData("bar", strs,typeName));
            typeList[i] = typeName;
        }
    ...
    @RequestMapping("/question17/year")
    public Map<String, Object> question17Year() {
        //故障最早发生日期
        Date earlyTime = new Date();
        //故障最晚发生日期
        Date lateTime = new Date(0,1,1);
        ...
        for (int i = 0; i < types.size(); i++) {
            ...
            //创建区间数组
            int[] strs = new int[range];
            String typeName = types.get(i).getZbBatch();
            //遍历全部数据记录
            for (New1qualityProblemData new1qualityProblemData : dataList) {
                //使用地址映射记录 同一时间同一种类的故障件的个数
                Date time = new1qualityProblemData.getTime();
...
class question17ChartData implements Serializable {
    public String type;
    public int[] data;
    public String name;

    public question17ChartData(String type, int[] data, String name) {
        this.type = type;
        this.data = data;
        this.name = name;
    }

    public question17ChartData() {
    }
