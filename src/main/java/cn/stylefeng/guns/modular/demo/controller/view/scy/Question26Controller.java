...
@RestController
@RequestMapping("/data")
public class Question26Controller extends ApiController {

    @Resource
    private New1qualityProblemDataService new1qualityProblemDataService;

    @RequestMapping("/question26")
    public Map<String, Object> question26() {
        //故障最早发生日期
        ...
        //获取故障件名称
        List<New1qualityProblemData> models = new1qualityProblemDataService.list(new QueryWrapper<New1qualityProblemData>()
                .select("faultparts_model")
                .groupBy("faultparts_model")
                .orderByDesc("faultparts_model")
        );
...
//            System.out.println(time);
        ...
        //遍历故障名称次
        List<Question26ChartData> series = new ArrayList<>();
        for (int i = 0; i < models.size(); i++) {
            //创建名称数量个数组 数组长度为
            //创建区间数组
            int[] strs = new int[range];
            String model = models.get(i).getFaultpartsModel();
            //遍历全部数据记录
            for (New1qualityProblemData new1qualityProblemData : dataList) {
                ...
                if (time.after(new Date(49,1,1))) {
                    ...
                    if (new1qualityProblemData.getFaultpartsModel().equals(model)) {
                        strs[year * 12 + month]++;
                    }
                }
            }
            series.add(new Question26ChartData("bar", strs,model));
            modelList[i] = model;
        }
    ...
    @RequestMapping("/question26/year")
    public Map<String, Object> question26Year() {
        //故障最早发生日期
        Date earlyTime = new Date();
        //故障最晚发生日期
        Date lateTime = new Date(0,1,1);
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            //创建区间数组
            int[] strs = new int[range];
            String modelname = models.get(i).getFaultpartsModel();
            //遍历全部数据记录
            for (New1qualityProblemData new1qualityProblemData : dataList) {
                //使用地址映射记录 同一时间同一种类的故障件的个数
                Date time = new1qualityProblemData.getTime();
                if(time.getYear()!=49) {
                    int year = time.getYear() - earlyYear;
                    if (new1qualityProblemData.getFaultpartsModel().equals(modelname)) {
                        strs[year]++;
                    }
                }
            }
            series.add(new Question26ChartData("bar", strs,modelname));
...
class Question26ChartData implements Serializable {
    public String type;
    public int[] data;
    public String name;

    public Question26ChartData(String type, int[] data, String name) {
        this.type = type;
        this.data = data;
        this.name = name;
    }

    public Question26ChartData() {
    }
